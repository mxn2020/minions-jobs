...
/**
 * @minions-jobs/cli — CLI for Minions jous
 *
...
import { customTypes } from '@minions-jobs/sdk';
...
program.name('jobs').description('Canonical schemas for job postings and extracted signals across freelance platforms').version('0.1.0');

program.command('info').description('Show project info').action(() => {
    console.log(chalk.bold('Minions jous'));
    console.log(chalk.dim('Canonical schemas for job postings and extracted signals across freelance platforms'));
    console.log('');
    console.log(`  SDK:    ${chalk.cyan('@minions-jobs/sdk')}`);
    console.log(`  CLI:    ${chalk.cyan('@minions-jobs/cli')}`);
    console.log(`  Python: ${chalk.cyan('minions_jobs')}`);
